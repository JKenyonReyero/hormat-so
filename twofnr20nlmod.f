...
*     ------------------------------------------------------------------
     ...
     4rhoisi,rhobc,rhobng,fkayar,fkayai,fkasor,fkasoi,fkaisr,fkaisi,    
     5fkaybg,vbe,wbe,vsoe,wsoe,vise,wise,vbcd(905,2),wbcd(905,2),       
     6vsov(905,2),wsov(905,2),pnloc(2),vcl(905,2)                                  
...
*     ut2=(ctemp1*ffi(m)+ctemp2*ff(m))/det                              
  ...
  398 continue                                                          
      ...
      do 400 m=1,nrmax                                                  
      ctemp1=-fact1*((vbcd(m,i)-1.0)+vcl(m,i))
     /       -fact2*((vbcd(m,k)-1.0)+vcl(m,k))               
...
*     if the same channel=2 wave function - already calculated
      ...
      lfchk=100+lf
      ljchk=100+ljf
      if((ljchk.eq.ljmem).and.(lfchk.eq.lfmem)) go to 70            
      ichnl=2
      lc=lf-1         
      drho=drhod(2)                                                     
      fkay=fk(2)                                                        
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      implicit real*8 (a-h,o-z)
      dimension    fc(90),gc(90),fcp(90),gcp(90)
